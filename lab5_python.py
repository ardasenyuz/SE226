def passGen(user_number):
    words = ["a", "ability", "able", "about", "above", "accept", "according", "account", "across", "act", "action",
             ...
             "can",
             "cancer", "candidate", "capital", "car", "card", "care", "career", "carry", "case", "catch", "cause",
             "cell",
             ...
             "final", "finally", "financial", "find", "fine", "finger", "finish", "fire", "firm", "first", "fish",
             "five",
             ...
             "list", "listen", "little", "live", "local", "long", "look", "lose", "loss", "lot", "love", "low",
             "machine",
             ...
             "option", "or", "order", "organization", "other", "others", "our", "out", "outside", "over", "own",
             "owner",
             ...
             "raise", "range", "rate", "rather", "reach", "read", "ready", "real", "reality", "realize", "really",
             "reason",
             ...
             "word", "work", "worker", "world", "worry", "would", "write", "writer", "wrong", "yard", "yeah", "year",
             "yes",
    ...
    print()
    password = ""
    for x in range(0, int(user_number), 1):
        random_string = random.choice(words)
        password = password + random_string

    return password

def rep_with_upper(given_string):
    import random

    random_index = random.randint(0, len(given_string)-1)
    upper_character= given_string[random_index].upper()
    temp_list= list(given_string)
    temp_list[random_index] = upper_character
    given_string = "".join(temp_list)

    return given_string

def swap_letters(given_string):
    temp_list = list(given_string)
    first_two = temp_list[0]+temp_list[1]
    last_two = temp_list[len(temp_list)-2]+temp_list[len(temp_list)-1]
    replaced_string = last_two + "".join(temp_list[2:(len(temp_list)-2)])+ first_two

    return replaced_string

def search_letter(first_string, second_string):
    boolean = second_string in first_string

    return boolean


print("Please choose a number between 3 and 7 inclusive.")
user_number = int(input())
while int(user_number) < 3 or int(user_number) > 7:
    print("Your number is not between 3 and 7. Please choose a number between 3 and 7 inclusive")
    user_number = int(input())

password = passGen(user_number)
print("first version of the password is: ",password)
password_upper = rep_with_upper(password)
print("second version of the password is: ",password_upper)
password_swapped = swap_letters(password_upper)
print("third version of the password is: ",password_swapped)
if (search_letter(password_swapped,"a")==1):
    print("password contains my name's first letter")
else:
    print("password does not contain my name's first letter")
