...
string passGen(int user_number) {
    ...
    cout<<"user number is: "<<user_number<<endl;

    string password;
    string random_string;
    for (int i=1; i<=user_number; i++){
        random_string = words[rand() % 1000];
...
string rep_with_upper(string given_string){
    int random_index = rand() % (given_string.length()-1);
    char upper_character = toupper(given_string[random_index]);
    char temp_list[given_string.length()];
    strcpy(temp_list, given_string.c_str());
    temp_list[random_index] = upper_character;
    for (int i =0; i<=sizeof(temp_list); i++){
        given_string[i]= temp_list[i];
    }

    return given_string;
}

string swap_letters(string given_string){
    char temp_list[given_string.length()];
    strcpy(temp_list, given_string.c_str());
    string first_two;
    first_two = first_two+temp_list[0]+temp_list[1];
    string last_two;
    last_two = last_two + temp_list[sizeof(temp_list)-2] + temp_list[sizeof(temp_list)-1];
    string convert_to_string ="";
    for (int i = 2; i<=sizeof(temp_list)-2; i++){
        convert_to_string = convert_to_string + temp_list[i];
    }
    string replaced_string = last_two + convert_to_string + first_two;

    return replaced_string;
}

bool search_letter(string first_string, string second_string){
    bool boolean;
    if (first_string.find('a')<first_string.length()){
        boolean = true;
    }else{
        boolean = false;
    }
...
int main(){
    cout<<"Please choose a number between 3 and 7 inclusive.";
    int user_number;
    cin>>user_number;
    while (user_number<3 || user_number>7){
        cout<<"Your number is not between 3 an 7. Please choose a number between 3 and 7."<<endl;
        cin>>user_number;
    }

    string password = passGen(user_number);
    cout<<"first version of the password is: "<<password<<endl;
    string password_upper = rep_with_upper(password);
    cout<<"second version of the password is: "<<password_upper<<endl;
    string password_swapped = swap_letters(password_upper);
    cout<<"third version of the password is: "<<password_swapped<<endl;
    if (search_letter(password_swapped, "a")==1){
        cout<<"password contains my name's first letter"<<endl;
    }else{
        cout<<"password does not contain my name's first letter"<<endl;
    }
}
